<template>
  <div>
    <v-btn
      ...
      :loading="isBusy"
      :title="$t(`button.upload['${target}'].title`)"
      :color="innerColor"
      @click="chooseFile"
      @dragover="dragOver"
      @dragleave="dragLeave"
...
export default {
  computed: {
    ...
    confirmFirmwareReset: {
      get() {
        return !this.confirmUpdate && this.confirmReset;
      },
      set(value) {
  ...
  methods: {
    ...
    getFirmwareName(fileName) {
      ...
      this.boards.forEach((board, index) => {
        if (
          board &&
          board.firmwareFileName &&
          (board.canAddress || index === 0)
        ) {
          const binRegEx = new RegExp(
            board.firmwareFileName.replace(/\.bin$/, "(.*)\\.bin"),
            "i",
          );
          const uf2RegEx = new RegExp(
            board.firmwareFileName.replace(/\.uf2$/, "(.*)\\.uf2"),
            "i",
          );
    ...
    getBinaryName(key, fileName) {
      ...
      this.boards.forEach((board) => {
        if (board && board[key]) {
          const binRegEx = new RegExp(
            board[key].replace(/\.bin$/, "(.*)\\.bin"),
            "i",
          );
          const uf2RegEx = new RegExp(
            board[key].replace(/\.uf2$/, "(.*)\\.uf2"),
            "i",
          );
          if (binRegEx.test(fileName) || uf2RegEx.test(fileName)) {
            result = board[key];
    ...
    async doUpload(files, zipName, startTime) {
      ...
      if (
        (this.target === "start" || this.target === "plugin") &&
        files.length !== 1
      ) {
          ...
          try {
            try {
              ...
              zip.forEach(function (file) {
                if (
                  !file.endsWith("/") &&
                  (file.split("/").length === 2 || target !== "filaments")
                ) {
                  zipFiles.push(file);
                }
              });
      ...
      for (let i = 0; i < files.length; i++) {
        let content = files[i],
          filename = Path.combine(this.destinationDirectory, content.name);
        if (
          ...
          if (Path.isSdPath("/" + content.name)) {
            ...
            this.updates.webInterface |= /index.html(\.gz)?/i.test(
              content.name,
            );
          } else if (
            this.connectorType === "rest" &&
            /\.deb$/.test(content.name)
          ) {
            // TODO improve this
            try {
            ...
            const bootloaderFileName = this.getBinaryName(
              "bootloaderFileName",
              content.name,
            );
            const iapFileNameSBC = this.getBinaryName(
              "iapFileNameSBC",
              content.name,
            );
            const iapFileNameSD = this.getBinaryName(
              "iapFileNameSD",
              content.name,
            );
            ...
            if (firmwareFileName) {
              ...
              !this.state.dsfVersion &&
              this.network.interfaces.some(
                (iface) => iface.type === NetworkInterfaceType.wifi,
              )
            ) {
              if (
                /DuetWiFiSocketServer(.*)\.bin/i.test(content.name) ||
                ...
                content.name.endsWith(".bin") ||
                content.name.endsWith(".uf2")
              ) {
                filename = Path.combine(
                  this.directories.firmware,
                  content.name,
      ...
      const askForUpdate =
        this.updates.firmwareBoards.length > 0 ||
        this.updates.wifiServer ||
        this.updates.wifiServerSpiffs ||
        this.updates.panelDue;
      ...
      try {
        ...
        console.debug("Upload completed", files);
        this.$emit("upload-complete", files);
      } catch (e) {
        this.uploading = false;
        console.warn("Upload failed", { files, error: e });
        this.$emit("upload-failed", { files, error: e });
        return;
      }
      this.uploading = false;
      ...
      if (askForUpdate) {
        ...
        this.selectedMachine === location.host &&
        this.updates.webInterface
      ) {
      ...
      for (let file of files) {
        ...
        if (
          !isPrinting(this.state.status) &&
          (fullName === Path.configFile ||
            fullName === configFile ||
            fullName === Path.boardFile)
        ) {
      ...
      if (zipName) {
        ...
        this.$makeNotification(
          "success",
          this.$t("notification.upload.success", [
            zipName,
            this.$displayTime(secondsPassed),
          ]),
          null,
        );
      }
    ...
    async startUpdate() {
      ...
      this.confirmReset =
        modules.indexOf("0") === -1 &&
        this.updates.firmwareBoards.findIndex((board) => board > 0) !== -1;
    },
    dragOver(e) {
      e.preventDefault();
      e.stopPropagation();
